<template>
    <div>
        ...
        <modal v-bind:mdShow="showError" v-on:close="closeModal">
            <p slot="message">
                用户已过期！请重新登录！
            </p>
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript:;" @click="showError = false">关闭</a>
            </div>
        </modal>
        <nav-footer />
    </div>
...
export default {
    ...
    methods: {
        //...mapActions(['updateCartCount', 'getCartList', 'delCartItem', 'cartEdit','editCheckAll']),
        delCartConfirm(item) {
            this.modalConfirm = true;
            this.delItem = item;
        },
        delCart() {
           // this.delCartItem(this.delItem).then((res) => {
              //  console.log(res);
        ...
        toggleCheckAll() {
            let flag = !this.checkAllFlag;  //避免改变计算属性的值时报错
            this.cartList.forEach((item) => {
